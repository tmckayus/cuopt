...
For gRPC protocol details, see `GRPC_ARCHITECTURE.md`.
...
```cpp
struct WorkerPipes {
  int to_worker_fd;           // Main → Worker: job data
  int from_worker_fd;         // Worker → Main: result data
  int incumbent_from_worker_fd; // Worker → Main: incumbent solutions
};
```
...
For multi-GB problems uploaded via gRPC streaming:

1. Server spools chunks to disk (`/tmp/cuopt_uploads/upload_<id>.bin`)
2. When upload completes, streams file contents to worker via pipe
3. Worker processes and writes result back via pipe
4. Server streams result to client via gRPC

This avoids holding entire payloads in server memory.
...
Client                     Worker
   ...
   │                          │ cudaMemcpy() to host
   │                          │      ↓
   │  cpu_lp_solution_t/      │ map_solution_to_proto()
   │  cpu_mip_solution_t ◄────│
```
...
cuopt_grpc_server [options]
  ...
  --port <port>              gRPC listen port (default: 9112)
  --workers <n>              Number of worker processes (default: 1)
  --max-message-mb <size>    Max gRPC message size in MiB (default: 256)
...
1. Monitor thread detects via `waitpid()`
2. Job marked as FAILED with error message
3. Worker can be respawned automatically
4. Other workers continue operating
...
2. Worker checks flag periodically during solve
3. If cancelled, worker stores CANCELLED result
4. Worker continues to next job (not restarted)
...
| Upload files | `/tmp/cuopt_uploads/` | After submission |
...
- Pipe buffer size is typically 64 KiB (Linux default)
...
| `/tmp/cuopt_uploads/` | Temporary storage for streamed uploads |
