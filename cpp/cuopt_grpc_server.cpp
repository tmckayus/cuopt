...
void worker_process(int worker_id)
{
  ...
  while (!shm_ctrl->shutdown_requested) {
    ...
    try {
      if (is_mip) {
        // Parse MIP request from protobuf
        cuopt::remote::SolveMIPRequest mip_request;
        if (!mip_request.ParseFromArray(request_data.data(), request_data.size())) {
          error_message = "Failed to parse MIP request protobuf";
        } else {
          // Extract problem and settings from protobuf
          cpu_optimization_problem_t<int, double> cpu_problem(&handle);
          mip_solver_settings_t<int, double> settings;

          map_proto_to_problem(mip_request.problem(), cpu_problem);
          map_proto_to_mip_settings(mip_request.settings(), settings);
          ...
          // Convert CPU problem to GPU problem for solving
          ...
          // Serialize CPU solution to protobuf
          cuopt::remote::MIPSolution pb_solution;
          map_mip_solution_to_proto(cpu_solution, &pb_solution);
          result_data.resize(pb_solution.ByteSizeLong());
          pb_solution.SerializeToArray(result_data.data(), result_data.size());
          success = true;
        }
      } else {
        // Parse LP request from protobuf
        cuopt::remote::SolveLPRequest lp_request;
        if (!lp_request.ParseFromArray(request_data.data(), request_data.size())) {
          error_message = "Failed to parse LP request protobuf";
        } else {
          // Extract problem and settings from protobuf
          cpu_optimization_problem_t<int, double> cpu_problem(&handle);
          pdlp_solver_settings_t<int, double> settings;

          map_proto_to_problem(lp_request.problem(), cpu_problem);
          map_proto_to_pdlp_settings(lp_request.settings(), settings);
          ...
          // Copy solution vectors from device to host
          ...
          map_lp_solution_to_proto(cpu_solution, &pb_solution);
          result_data.resize(pb_solution.ByteSizeLong());
          pb_solution.SerializeToArray(result_data.data(), result_data.size());
          success = true;
        }
...
class CuOptRemoteServiceImpl final : public cuopt::remote::CuOptRemoteService::Service {
 public:
  ...
  // For large results, clients should use StreamResult instead.
                   ...
                   cuopt::remote::ResultResponse* response) override
  {
    ...
    // Check if result is too large for unary RPC.
    // Return RESOURCE_EXHAUSTED to signal client to use StreamResult instead.
    const int64_t max_bytes =
      (config.max_message_mb <= 0) ? -1 : (static_cast<int64_t>(config.max_message_mb) * kMiB);
    if (max_bytes > 0 && static_cast<int64_t>(it->second.result_data.size()) > max_bytes) {
      std::string msg = "Result size (" + std::to_string(it->second.result_data.size()) +
                        " bytes) exceeds max message size (" + std::to_string(max_bytes) +
                        " bytes). Use StreamResult RPC instead.";
      if (config.verbose) {
                           ...
                           cuopt::remote::WaitResponse* response) override
  {
    ...
    response->set_result_size_bytes(static_cast<int64_t>(result_data.size()));
