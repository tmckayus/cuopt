                                ...
                                cuopt::remote::ChunkedProblemHeader* header)
{
  ...
  rh->set_index_type(sizeof(i_t) == 4 ? cuopt::remote::INT32 : cuopt::remote::INT64);
  rh->set_float_type(sizeof(f_t) == 4 ? cuopt::remote::FLOAT32 : cuopt::remote::DOUBLE);
...
MessageStream problem_to_messages(const cpu_optimization_problem_t<i_t, f_t>& problem,
                                  ...
                                  bool enable_incumbents)
{
  bool is_mip = (mip_settings != nullptr);

  MessageStream stream;
  cuopt::remote::PipeEnvelope hdr_env;
  auto* header = hdr_env.mutable_chunked_header();

  auto* rh = header->mutable_header();
  rh->set_version(1);
  rh->set_problem_type(is_mip ? cuopt::remote::MIP : cuopt::remote::LP);
  rh->set_index_type(sizeof(i_t) == 4 ? cuopt::remote::INT32 : cuopt::remote::INT64);
  rh->set_float_type(sizeof(f_t) == 4 ? cuopt::remote::FLOAT32 : cuopt::remote::DOUBLE);

  header->set_maximize(problem.get_sense());
  header->set_objective_scaling_factor(problem.get_objective_scaling_factor());
  header->set_objective_offset(problem.get_objective_offset());
  header->set_problem_name(problem.get_problem_name());
  header->set_objective_name(problem.get_objective_name());
  header->set_enable_incumbents(enable_incumbents);

  if (lp_settings) { map_pdlp_settings_to_proto(*lp_settings, header->mutable_lp_settings()); }
  if (mip_settings) { map_mip_settings_to_proto(*mip_settings, header->mutable_mip_settings()); }
  ...
  append_array(stream, cuopt::remote::FIELD_A_VALUES, values);
  append_array(stream, cuopt::remote::FIELD_A_INDICES, indices);
  append_array(stream, cuopt::remote::FIELD_A_OFFSETS, offsets);
  append_array(stream, cuopt::remote::FIELD_C, obj);
  append_array(stream, cuopt::remote::FIELD_VARIABLE_LOWER_BOUNDS, var_lb);
  append_array(stream, cuopt::remote::FIELD_VARIABLE_UPPER_BOUNDS, var_ub);
  append_array(stream, cuopt::remote::FIELD_CONSTRAINT_LOWER_BOUNDS, con_lb);
  append_array(stream, cuopt::remote::FIELD_CONSTRAINT_UPPER_BOUNDS, con_ub);
  append_array(stream, cuopt::remote::FIELD_B, b);

  // Row types and variable types as byte arrays
  auto row_types = problem.get_row_types_host();
  if (!row_types.empty()) {
    std::vector<uint8_t> rt_bytes(row_types.begin(), row_types.end());
    append_array(stream, cuopt::remote::FIELD_ROW_TYPES, rt_bytes);
  }
  ...
  if (!var_types.empty()) {
    ...
    append_array(stream, cuopt::remote::FIELD_VARIABLE_TYPES, vt_bytes);
  }
  ...
  if (problem.has_quadratic_objective()) {
    ...
    append_array(stream, cuopt::remote::FIELD_Q_VALUES, q_values);
    append_array(stream, cuopt::remote::FIELD_Q_INDICES, q_indices);
    append_array(stream, cuopt::remote::FIELD_Q_OFFSETS, q_offsets);
  }

  // Variable/row names as null-separated byte blobs
  ...
  append_array(stream, cuopt::remote::FIELD_VARIABLE_NAMES, var_names_blob);
  append_array(stream, cuopt::remote::FIELD_ROW_NAMES, row_names_blob);
...
void messages_to_problem(const MessageStream& messages,
                         ...
                         bool& enable_incumbents)
{
  const auto& header = find_problem_header(messages);

  if (header.has_lp_settings()) { map_proto_to_pdlp_settings(header.lp_settings(), lp_settings); }
  if (header.has_mip_settings()) { map_proto_to_mip_settings(header.mip_settings(), mip_settings); }
  enable_incumbents = header.enable_incumbents();

  // Reconstruct problem from header scalars + stream arrays
  // (reuses the existing map_chunked_arrays_to_problem which handles both header scalars
  // and array data keyed by field_id)
  std::map<int32_t, std::vector<uint8_t>> arrays;
  for (const auto& env : messages) {
    if (env.has_array_data()) {
      const auto& ad  = env.array_data();
      const auto& raw = ad.data();
      arrays[ad.field_id()].assign(raw.begin(), raw.end());
    }
  }

  map_chunked_arrays_to_problem(header, arrays, problem);
}

// ============================================================================
// Transport adaptors: gRPC protobuf -> MessageStream
// ============================================================================
...
{
  cpu_optimization_problem_t<i_t, f_t> cpu_problem;
  map_proto_to_problem(request.problem(), cpu_problem);

  pdlp_solver_settings_t<i_t, f_t> lp_settings;
  map_proto_to_pdlp_settings(request.settings(), lp_settings);

  return problem_to_messages(
    cpu_problem, &lp_settings, static_cast<const mip_solver_settings_t<i_t, f_t>*>(nullptr), false);
}
...
MessageStream mip_request_proto_to_stream(const cuopt::remote::SolveMIPRequest& request)
{
  ...
  mip_solver_settings_t<i_t, f_t> mip_settings;
  map_proto_to_mip_settings(request.settings(), mip_settings);

  bool enable_inc = request.has_enable_incumbents() ? request.enable_incumbents() : true;
  return problem_to_messages(cpu_problem,
...
template void map_chunked_arrays_to_problem(
  ...
  const std::map<int32_t, std::vector<uint8_t>>& arrays,
  cpu_optimization_problem_t<int32_t, float>& cpu_problem);
template MessageStream problem_to_messages(
  const cpu_optimization_problem_t<int32_t, float>& problem,
  const pdlp_solver_settings_t<int32_t, float>* lp_settings,
  const mip_solver_settings_t<int32_t, float>* mip_settings,
  bool enable_incumbents);
template void messages_to_problem(const MessageStream& messages,
                                  cpu_optimization_problem_t<int32_t, float>& problem,
                                  pdlp_solver_settings_t<int32_t, float>& lp_settings,
                                  mip_solver_settings_t<int32_t, float>& mip_settings,
                                  bool& enable_incumbents);
template MessageStream lp_request_proto_to_stream<int32_t, float>(
  const cuopt::remote::SolveLPRequest& request);
template MessageStream mip_request_proto_to_stream<int32_t, float>(
...
template MessageStream problem_to_messages(
  ...
  const pdlp_solver_settings_t<int32_t, double>* lp_settings,
  const mip_solver_settings_t<int32_t, double>* mip_settings,
  bool enable_incumbents);
template void messages_to_problem(const MessageStream& messages,
                                  cpu_optimization_problem_t<int32_t, double>& problem,
                                  pdlp_solver_settings_t<int32_t, double>& lp_settings,
                                  mip_solver_settings_t<int32_t, double>& mip_settings,
                                  bool& enable_incumbents);
template MessageStream lp_request_proto_to_stream<int32_t, double>(
  const cuopt::remote::SolveLPRequest& request);
template MessageStream mip_request_proto_to_stream<int32_t, double>(
  const cuopt::remote::SolveMIPRequest& request);
#endif
